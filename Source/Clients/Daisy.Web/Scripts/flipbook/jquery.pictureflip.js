(function($, w, d, undefined){
	...
	$.pageFlip.prototype = {  
		...
		append : function() {
			...
			this.$element.append('<div class="icon_prev"></div><div class="icon_next"></div>');
			...
			if(Modernizr.csstransforms3d) {
				...
				this.$element.find('.slide').each(function() {
					...
					//$(this).find('.img').attr('style', 'background-size: contain !important');
					//$(this).find('.img:last-of-type').attr('style', 'background-size: contain !important');
					$(this).find('.img').css({
						'background' : 'url("'+image+'") no-repeat center center fixed', 						
						'transition-duration' : ($this.options.time/2)+'s, '+($this.options.time/2)+'s'
					});									
					$(this).find('.img')[0].style.setProperty('background-size', 'contain', 'important');
					// The last image should be on the right side, so adjust that					
					$(this).find('.img:last-of-type').css({
						'right' : '0', 
						'left' : 'auto'						
					});	
					$(this).find('.img:last-of-type')[0].style.setProperty('background-size', 'contain', 'important');
				});
		...
		events : function() {
			...
			$('.icon_next', $this.$element).on("click", function() {
				nextClick(false)
			});
			
			// The previous slide button
			$('.icon_prev', $this.$element).on("click", function() {
				prevClick(false)
			});
			...
			$(w).on("hover", function() {
				...
				if(hovering === false) {
					...
					autoslide = setInterval(function() {
						...
						$this.$element.find('.icon_next').click();
